...
The daemon searches for `Codex/Queue/BN-Sync/<PROJECT_ID>`, calls `Zotero.BetterNotes.api.$export.syncMDBatch` one note at a time, removes the queue tag only after verified success, and adds `Codex/BN-Synced/<PROJECT_ID>`.
...
If a note is already synced to this project root, the selected action should report `already=1` rather than creating another note. If it is synced to a different root, the script attempts to re-register it under the current `ROOT_DIR`.
