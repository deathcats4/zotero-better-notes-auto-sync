...
Prefer the queue daemon for hands-off Codex workflows. It runs on `mainWindowLoad`, searches for `Codex/Queue/BN-Sync/<PROJECT_ID>`, calls Better Notes one note at a time only when registration/export is needed, removes the queue tag only after sync verification, and adds `Codex/BN-Synced/<PROJECT_ID>` only on verified success. It also periodically asks Better Notes to compare/sync already-linked project notes, because Better Notes' own timer only runs while the Zotero main window has focus and may skip active visible notes. Daemon timer and busy state are keyed by `PROJECT_ID`, so multiple projects can stay resident at the same time; re-running the same `PROJECT_ID` daemon clears and replaces the old timer so configuration changes take effect.
...
- In the queue daemon, `ENABLE_PROJECT_LINKED_NOTE_AUTOSYNC` may call Better Notes `hooks.onSyncing()` for already-linked project notes that still point to `ROOT_DIR` and have an existing Markdown file. This is not a forced `syncMDBatch` export; it uses Better Notes' bidirectional compare/import/export logic. Active visible notes are only included after their content digest has stayed unchanged for `ACTIVE_NOTE_SETTLE_SECONDS`.
