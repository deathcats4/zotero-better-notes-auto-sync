...
- Treat one Zotero note as owned by one sync project by default. If a note has another project's `Codex/BN-Synced/*`, `Codex/BN-Note/*`, marker, or Better Notes status under another root, fail with `cross_project_note_conflict`; use a separate child note per project unless the user explicitly enables migration.
